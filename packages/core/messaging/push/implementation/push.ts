...
export class PushAPIModule extends BasePushAPI {
  ...
  async sendNotification(notification: notificationType): Promise<any> {
    try {
      const notificationResponse = await PushAPI.payloads.sendNotification({
        signer: notification.signer,
        type: notification.type,
        identityType: notification.identityType,
        notification: {
        ...
        payload: {
          ...
          cta: notification.payload?.cta,
          img: notification.payload?.img,
        },
        recipients: notification.recipients,
        channel: `eip155:${notification.chainId}:${notification.channel}`,
